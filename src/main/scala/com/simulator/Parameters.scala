...
object Parameters {
  ...
  def getFinalStats(inputMageStats: MageStats, statBuffs: StatBuffs = StatBuffs(), consumables: Consumables = Consumables()): MageStats = {
    ...
    val intCrit = (intellect - inputMageStats.intellect).toDouble / 80d
    ...
    val crit = inputMageStats.critChance + statBuffs.ijotc + statBuffs.mka + statBuffs.tow + critRatingToChance(consumables.bwo._2 + consumables.aad._1) + 2 + intCrit

    println(crit)

    val hit = inputMageStats.hitChance + statBuffs.hp
